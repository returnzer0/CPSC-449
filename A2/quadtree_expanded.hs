...
--
data QuadTree  = Node QuadTree QuadTree QuadTree QuadTree | Fill (Int, Int, Int) | NilT deriving (Show)
data ImageTree = Image Int Int QuadTree deriving (Show)
-- Width Height Quadtree
--
...
createTree list = Image width height (createQTree list)
   where
    height = length list
...
divideEqually list = (topLeft, topRight, bottomLeft, bottomRight)
  where
    ...
    halfW       = width `div` 2
    top         = take halfH list
    bottom      = drop halfH list
    topLeft     = [take halfW x | x <- top ]
    topRight    = [drop halfW x | x <- top ]
    bottomLeft  = [take halfW x | x <- bottom]
    bottomRight = [drop halfW x | x <- bottom]

-- Part 3
-- We dont need to include NilT in the f-functions because the treeMap funciton takes care of it.
...
mirror (Image width height node) = (Image width height (treeMap mirrorHelper node))
...
grayscale (Image width height node) = (Image width height (treeMap grayscaleHelper node))
...
-- Part 4
-- We only create the prefix and the suffix in this function, the nodes are visited and their tags are created in the helper function.
toHTML :: ImageTree -> String
toHTML (Image width height q) = "<html><head></head><body>\n" ++
                     "<svg width=\"" ++ (show width) ++ 
                     "\" height=\"" ++ (show height) ++ "\">" ++
                     (toHTMLHelper 0 0 width height q) ++
                     "</svg>\n</html>"

toHTMLHelper :: Int -> Int -> Int -> Int -> QuadTree -> String
toHTMLHelper _ _ _ _ (NilT)                    = ""
toHTMLHelper x y width height (Fill (a, b, c)) = "<rect x=" ++ (show x) ++ 
                                          " y=" ++ (show y) ++ 
                                          " width=" ++ (show width) ++ 
                                          " height=" ++ (show height) ++ 
                                          " stroke=\"black\"" ++
...
toHTMLHelper x y width height (Node a b c d)  = topLeft ++ topRight ++ bottomLeft ++ bottomRight
   where
    -- Using the correct offsets for each part of the Image.
    newWidth    = width `div` 2
    newHeight   = height `div` 2
    topLeft     = toHTMLHelper  x              y              newWidth newHeight a
    topRight    = toHTMLHelper (x + newWidth)  y              newWidth newHeight b
    bottomLeft  = toHTMLHelper  x             (y + newHeight) newWidth newHeight c
    bottomRight = toHTMLHelper (x + newWidth) (y + newHeight) newWidth newHeight d
